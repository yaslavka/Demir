...
import {yes} from "./yes";
import {dopuslugi} from "../../public/Home/HomeComponents/SearchHotel/data";
...
export default function CustomizedHook() {
    const [values, setValues]=useState(dopuslugi.uslugis)
                ...
                <Listbox {...getListboxProps()}>
                    {groupedOptions.map((option, index) => (
                        <li {...getOptionProps({ option, index })}>
                            <span id={option.id} aria-checked={option.checked} onClick={()=>chengedopuslugii(option.id)}>{option.label}</span>
