...
function Paket({pdnyatie, setPdnyatie}){
    const [myobjekt, setMyobjekt] = useState([])
    const [inputp, setInputp] = useState({paket: 0})
    ...
    const pdnyatiein =(e)=>{
        setInputp({paket: e.target.value})
    }
    const hotelIdsel =(e)=>{
    ...
    const broneceng = async ()=>{
        ...
        let result = await confirm({
            title:`${t('Подтвердите Пакетное размещение')}`,
            message: `${t('Пакетное размещение на')} ${planetLength} ${declOfNum(planetLength, [
                `${t('день')}`,
                `${t('дня')}`,
                `${t('дней')}`,
        ...
        if (result) {
            api
                .postBronenomer({hotelId:hotelId, paket:podnyatie})
                .then((response)=>{
