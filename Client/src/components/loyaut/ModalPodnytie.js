...
function Podnytie({pdnyatie, setPdnyatie}){
    const [myobjekt, setMyobjekt] = useState([])
    const [inputp, setInputp] = useState({podnyatie: 0})
    ...
    const pdnyatiein =(e)=>{
        setInputp({podnyatie: e.target.value})
    }
    const hotelIdsel =(e)=>{
    ...
    const broneceng = async ()=>{
        ...
        let result = await confirm({
            ...
            message: `${t('Поднятие в списке на')} ${planetLength} ${declOfNum(planetLength, [
                `${t('день')}`,
                `${t('дня')}`,
                `${t('дней')}`,
        ...
        if (result) {
            api
                .postBronenomer({hotelId:hotelId, podnyatie:podnyatie})
                .then((response)=>{
                        ...
                        <Form onSubmit={(e)=>e.preventDefault()}
                            ...
                            <fieldset>
                                <div className="form-group">
                                    <label htmlFor="podnyatie" className="sr-only">Укажите количество дней:</label>
                                    <input type="number" name="podnyatie"
                                           value={podnyatie}
