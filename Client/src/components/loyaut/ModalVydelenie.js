...
function Vydelenie({pdnyatie, setPdnyatie}){
    const [myobjekt, setMyobjekt] = useState([])
    const [inputp, setInputp] = useState({vydylenie: 0})
    ...
    const pdnyatiein =(e)=>{
        setInputp({vydylenie: e.target.value})
    }
    const hotelIdsel =(e)=>{
    ...
    const broneceng = async ()=>{
        ...
        let result = await confirm({
            title:`${t('Подтвердите Выдиление')}`,
            message: `${t('Выдиление в списке на')} ${planetLength} ${declOfNum(planetLength, [
                `${t('день')}`,
                `${t('дня')}`,
                `${t('дней')}`,
                        ...
                        <Form onSubmit={(e)=>e.preventDefault()}
                            ...
                            <fieldset>
                                <div className="form-group">
                                    <label htmlFor="vydylenie" className="sr-only">Укажите количество дней:</label>
                                    <input type="number" name="vydylenie"
                                           value={podnyatie}
